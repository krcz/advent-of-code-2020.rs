...
impl TriSpace {
    fn new(xn: usize, yn: usize, round: u32) -> TriSpace {
        ...
        let wm = zm * (1 + 2 * round as usize);
        let size = wm * (1 + 2 * round as usize);
        let mut data = BitVec::with_capacity(size);
        data.grow(size, false);
        TriSpace {xn, yn, ym, zm, wm, round: round as i32, data}
    }
    ...
    fn zwmap(&self, z: i32) -> Option<usize> {
    ...
    fn get(&self, x: i32, y: i32, z: i32, w: i32) -> bool {
        ...
        if let Some(xx) = self.xmap(x) {
            if let Some(yy) = self.ymap(y) {
                if let Some(zz) = self.zwmap(z) {
                    if let Some(ww) = self.zwmap(w) {
                        return self.data.get(xx + yy * self.ym + zz * self.zm + ww * self.wm).unwrap();
                    }
                }
            }
        }
    ...
    fn set(&mut self, x: i32, y: i32, z: i32, w:i32, value: bool) {
        ...
        let zz = self.zwmap(z).unwrap();
        let ww = self.zwmap(w).unwrap();
        self.data.set(xx + yy * self.ym + zz * self.zm + ww * self.wm, value);
    }
...
impl<'a> Neighbourhood<'a> {
    fn iter(&'a self) -> impl Iterator<Item = bool> + 'a {
        (0 as i32 .. 81 as i32).filter_map(move |k: i32| -> Option<bool> {
            if k == 40 {
                None
            } else {
                Some(self.board.get(self.x - 1 + k % 3, self.y - 1 + k / 3 % 3, self.z - 1 + k / 9 % 3, self.w - 1 + k / 27))
            }
        })
    }
}
...
fn main() {
    ...
    for x in 0..xn {
        for y in 0..yn {
            board.set(x as i32, y as i32, 0, 0, flat_board[y][x] == '#')
        }
    }
    ...
    for round in 1..=max_round {
        ...
        for x in nb.minx()..=nb.maxx() {
            for y in nb.miny()..=nb.maxy() {
                for z in nb.minz()..=nb.maxz() {
                    for w in nb.minw()..=nb.maxw() {
                        let occ: u32 = Neighbourhood { board: &board, x, y, z, w}.iter().filter(|&b| b).count() as u32;
                        nb.set(x, y, z, w, if board.get(x, y, z, w) { occ == 2 || occ == 3 } else { occ == 3 })
                    }
                }
            }
        }
        ...
        for y in nb.miny()..=nb.maxy() {
            println!("{}", (nb.minx()..=nb.maxx()).map(|x| if nb.get(x, y, 0, 0) {'#'} else {'.'}).collect::<String>());
        }
        println!();
